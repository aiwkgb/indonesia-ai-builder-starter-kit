# Landing Page Brief Template

Template ini membantu pemula merencanakan landing page sederhana sebelum menulis copy, membuat desain, atau membangun halaman dengan bantuan AI.

## Cara menggunakan template ini

Isi brief ini sebelum meminta AI menulis copy atau membuat halaman. Jawaban singkat lebih baik daripada menunggu semua detail sempurna.

Gunakan brief ini untuk menjaga tujuan, audiens, offer, CTA, dan klaim tetap jelas.

## 1. Ringkasan landing page

- Nama project/brand:
- Deskripsi singkat:
- Tujuan landing page:
- Target utama:
- Status project saat ini:

## 2. Target audiens

- Siapa audiens utama?
- Masalah/kebutuhan utama mereka:
- Apa yang sudah mereka pahami?
- Apa yang masih membuat mereka ragu?

## 3. Offer atau value proposition

Jelaskan manfaat secara realistis. Hindari janji hasil yang belum bisa dibuktikan.

- Apa yang ditawarkan?
- Kenapa ini berguna?
- Apa hasil realistis yang bisa diharapkan?
- Apa yang tidak boleh dijanjikan berlebihan?

## 4. Main message

- Headline utama:
- Subheadline:
- Pesan utama yang harus dipahami pengunjung:
- Satu kalimat sederhana tentang landing page ini:

## 5. Struktur halaman

Centang section yang benar-benar dibutuhkan untuk versi awal.

- [ ] Hero section
- [ ] Problem section
- [ ] Solution section
- [ ] Benefits
- [ ] How it works
- [ ] Features or contents
- [ ] Social proof or trust notes
- [ ] FAQ
- [ ] Final CTA

Gunakan hanya section yang relevan. Landing page sederhana tidak harus memakai semuanya.

## 6. CTA

- CTA utama:
- CTA alternatif:
- Link tujuan CTA:
- Aksi yang diharapkan dari pengunjung:

## 7. Trust and proof

- [ ] Testimonial
- [ ] Portfolio/example
- [ ] Screenshot
- [ ] Case study
- [ ] FAQ
- [ ] Clear disclaimer
- [ ] Contact or support info

Masukkan hanya proof yang nyata dan bisa dicek.

## 8. Content and assets needed

Gunakan bagian ini untuk mengecek bahan yang perlu disiapkan sebelum build.

- [ ] Logo
- [ ] Brand colors
- [ ] Product image/mockup
- [ ] Copywriting
- [ ] Icons
- [ ] Screenshots
- [ ] FAQ answers
- [ ] Contact link
- [ ] Privacy or terms link if needed

## 9. AI-assisted workflow notes

- AI tools yang digunakan:
- Bagian yang boleh dibantu AI:
- Bagian yang harus direview manusia:
- Risiko klaim yang perlu dicek:
- Catatan manual review:

## 10. Simple review checklist

- [ ] Apakah tujuan landing page jelas?
- [ ] Apakah audiens utama jelas?
- [ ] Apakah CTA mudah ditemukan?
- [ ] Apakah klaim masih realistis?
- [ ] Apakah ada data pribadi atau secret yang tidak sengaja masuk?
- [ ] Apakah halaman bisa dipahami pemula dalam beberapa menit?

## Short English summary

This template helps beginners prepare a simple landing page brief before writing copy, designing, or building with AI-assisted workflows.
